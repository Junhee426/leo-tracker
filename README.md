...
`crosscheck_points`는 위성망별로 확보된 **모든** 날짜 명시 독립 수치(카탈로그 추적,
사업자 배치 발표, 규제 승인, 계획 목표)를 `metric` 태그와 함께 나열합니다. 위성망 상세
페이지의 Cross-check 패널은 이 값들을 같은 `metric`끼리만 묶어 비교합니다 — 예를 들어
Amazon Leo는 두 개의 독립된 배치 발표(396 / ~400)가 서로 근접해 "일치" 배지가 붙고,
Guowang은 CelesTrak 카탈로그 추적 수와 우크라이나 추적센터의 식별 위성 수가 같은
`tracked` 지표로 비교됩니다. 반면 승인 수량(FCC 15,000)과 배치 수량(SpaceX 9,600)처럼
정의가 다른 값은 서로 다른 지표 그룹으로 유지되어 "상이"로 오판되지 않습니다. 같은
지표에서 출처가 하나뿐이면 "단일 출처 · 교차검증 대기"로 표시됩니다.
